...
const LetterGlitch: React.FC<LetterGlitchProps> = ({
  ...
  const activeColors = glitchColors || colors;
  const activeSpeed = glitchSpeed !== undefined ? glitchSpeed : speed;
  const activeCenterVignette = centerVignette !== undefined ? centerVignette : showCenterVignette;
  const activeOuterVignette = outerVignette !== undefined ? outerVignette : showOuterVignette;
  ...
  const animate = () => {
    ...
    const now = Date.now();
    if (now - lastGlitchTime.current >= activeSpeed) {
      updateLetters();
      drawLetters();
  ...
  return (
    <div ref={containerRef} style={containerStyle} className={className} aria-hidden="true">
      <canvas ref={canvasRef} style={canvasStyle} />
      {activeOuterVignette && <div style={outerVignetteStyle} />}
      {activeCenterVignette && <div style={centerVignetteStyle} />}
    </div>
  );
};
